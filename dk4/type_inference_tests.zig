...
test "array: multiple fills same level" {
    ...
    try std.testing.expect(infer_res.hasErrors());
    var found = false;
    for (infer_res.errors.items) |e| if (e.error_ == .multiple_fills_in_array_literal) { found = true; break; };
...
test "array: inconsistent elem types [1,2,true]" {
    ...
    try std.testing.expect(infer_res.hasErrors());
    var found = false;
    for (infer_res.errors.items) |e| if (e.error_ == .inconsistent_elem_types_in_array_lit) { found = true; break; };
    try std.testing.expect(found);
}
...
test "array: inconsistent inner dim [[1,2],[1,2,3]]" {
    ...
    try std.testing.expect(infer_res.hasErrors());
    var found = false;
    for (infer_res.errors.items) |e| if (e.error_ == .inconsistent_inner_dim) { found = true; break; };
    try std.testing.expect(found);
}
